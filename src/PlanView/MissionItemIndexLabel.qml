...
Canvas {
    ...
    /// When set (from loaded Daya ``.plan``), overrides vehicle-id-based marker color.
    property string planVisualColor:       ""
    /// When set, used for Daya multi-UAV marker tint when ``planVisualColor`` is empty (instead of activeVehicle).
    property var mapVehicle: null

    property var    color:                  checked ? "green" : (child ? qgcPal.mapIndicatorChild : (planVisualColor.length > 0 ? planVisualColor : _dayaVehicleColor()))
    ...
    function _dayaVehicleColor() {
        ...
        const v = mapVehicle || QGroundControl.multiVehicleManager.activeVehicle
        const vid = v ? v.id : 0
